...
@dataclass
class SpeckitTasksStep:
    name: str = "speckit-tasks"
    skip: bool = False

    def execute(self, context: PipelineContext) -> None:
        def action() -> None:
            result = context.require_result()
            prompt = agent_execution._build_agent_prompt(context, result, "tasks")
            session_id, resume_session = agent_execution._resolve_agent_session(context, "tasks")
            ...
            agent_execution._store_agent_output(context, result, "tasks", raw_output)
