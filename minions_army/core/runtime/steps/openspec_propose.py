...
@dataclass
class OpenspecProposeStep:
    name: str = "openspec-propose"
    skip: bool = False

    def execute(self, context: PipelineContext) -> None:
        def action() -> None:
            result = context.require_result()
            prompt = agent_execution._build_agent_prompt(context, result, "propose")
            session_id, resume_session = agent_execution._resolve_agent_session(context, "propose")
            ...
            agent_execution._store_agent_output(context, result, "propose", raw_output)
