...
@dataclass
class SpeckitPlannerStep:
    name: str = "speckit-planner"
    skip: bool = False

    def execute(self, context: PipelineContext) -> None:
        def action() -> None:
            result = context.require_result()
            prompt = agent_execution._build_agent_prompt(context, result, "planner")
            session_id, resume_session = agent_execution._resolve_agent_session(context, "planner")
            ...
            agent_execution._store_agent_output(context, result, "planner", raw_output)
