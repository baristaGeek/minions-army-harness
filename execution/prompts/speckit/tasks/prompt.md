Create the task breakdown from the completed plan.
...
- Keep the task breakdown actionable and narrowly scoped.
- Do not start implementation work in this stage.
...
- Use the selected SDD framework `speckit` and run `speckit-tasks` before producing the JSON response.
- Do not continue unless the plan artifact exists and the tasks artifact is created or updated successfully for the selected framework.
